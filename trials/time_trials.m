function [ev_fn_time, gefe_fn_time] = time_trials(n, num_trials, eigenvector_function)
...
%               [y1, y2] = time_trials(n, number_of_trials, @eig);
    ...
    ev_fn = @() run_trials_eigenvector_function(H, n, num_trials, eigenvector_function);
    ev_fn_time = timeit(ev_fn);
    
    disp('------------------------------------');
    
    fprintf('   Computing GetEigenvectorFromEigenvalues \n');
    gefe_fn = @() run_trials_gefe_function(H, n, num_trials);
    gefe_fn_time = timeit(gefe_fn);
    
    disp('-----------------------------------------------------');
end

% TODO(cgyuryik): documentation.
function run_trials_eigenvector_function(H, n, num_trials, eigenvector_function)
    for k = 1:num_trials
...
% TODO(cgyuryik): documentation.
function run_trials_gefe_function(H, n, num_trials)
    eigvals = eig(H);
    ii = 1:n;
    for k = 1:num_trials
